...
public class CVELinking2 {
    ...
    public static Model generateLinkingCVETOCPE(Model CVEModel, Model CPEModel, String fileName) throws IOException {
        ...
        while (QResult1.hasNext()) {
            ...
            ArrayList<String>[] CPEArrayofList =
                    getCPEResourceFilterStatement(filterCPE + "?cpeId = \"0\"", CPEModel);
            ...
            //if CPE is empty or no ac.at.tuwien.ifs.sepses.linking to CPE at all
            if (CPEArrayofList[0].size() == 0) {
                System.out.println(
                        cveId + ": CPE is empty, cpe= " + CPEArrayofList[0].size() + "& cpeInCVE =" + filterCPEArray
                                .size());
                int i = 0;
                ...
                //if CVE has complete CPE ac.at.tuwien.ifs.sepses.linking
                if (filterCPEArray.size() == CPEArrayofList[0].size()) {
                    System.out.println(cveId + ": CPE is complete, cpe =" + CPEArrayofList[0].size() + "& cpeInCVE ="
                            + filterCPEArray.size());
                    int j = 0;
                    ...
                    //if CPE is not completely ac.at.tuwien.ifs.sepses.linking (partial ac.at.tuwien.ifs.sepses.linking to CVE)
                    System.out.println(
                            cveId + ": CPE is not complete, cpe =" + CPEArrayofList[0].size() + "& cpeInCVE ="
                                    + filterCPEArray.size());
                    ...
                    while (filterCPEArray.size() > l) {
                        if (!CPEArrayofList[1].contains(filterCPEArray.get(l))) {
                            ...
                            // System.out.println(CVER.toString()+" has no ac.at.tuwien.ifs.sepses.linking !! ");
                            Resource resSn = NolinkingModelCVECPE.createResource(cveId.toString());
        ...
        String fileNameNL = "output/ac.at.tuwien.ifs.sepses.linking/CVETOCPE_" + fileName + "_NoLinking.log.ttl";
    ...
    public static Model generateLinkingCVETOCWE(Model CVEModel, Model CWEModel, String fileName) throws IOException {
        ...
        ArrayList<String>[] CWEResArray =
                getCWEResourceFilterStatement(filterStatement2 + "?cweId = \"0\"", CWEModel);
        ...
        while (sidQResult4.hasNext()) {
            ...
            while (CWEResArray[1].size() > i) {
                ...
                if (CWEResArray[1].get(i).equals(CVECWE4.toString().substring(4))) {
                    ...
                    CWEfound++;
                    System.out.println(
                            "CWE Found, generate ac.at.tuwien.ifs.sepses.linking " + CVERes4.toString() + "to " + CWEResArray[0].get(i));
            ...
            if (CWEfound < 1) {
                ...
                System.out.println(
                        "CWE Not Found, generate No ac.at.tuwien.ifs.sepses.linking " + CVERes4.toString() + "to " + CVECWE4.toString());
                CWENotFound++;
            } else {
                CWEFound++;
            }
        ...
        String fileNameNL = "output/ac.at.tuwien.ifs.sepses.linking/CVETOCWE_" + fileName + "_NoLinking.log.ttl";
        // FileWriter rdfLinking = new FileWriter(fileName);
