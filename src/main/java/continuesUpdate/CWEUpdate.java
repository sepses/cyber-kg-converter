...
public class CWEUpdate {
	...
	private static void deleteCWE(String cyberKnowledgeEp, String graphname, String CWEId) {
		 String cweRes = "<"+CWEId+">";
		 //level 1 deep
		 String deleteQuery1 = "with  <"+graphname+"> DELETE { ?s ?p ?o }  \r\n" + 
		 		              "WHERE { ?s ?p ?o. "
		 		              		+ "filter (?s = "+cweRes+")}";
		 //level 2 deep
		 String deleteQuery2 = "with <"+graphname+"> DELETE { ?o ?p1 ?o1 }  \r\n" + 
	              "WHERE { ?s ?p ?o. "
	              		+ " ?o ?p1 ?o1. "
	              		+ "filter (?s = "+cweRes+")}";
		 //level 3 deep
		 String deleteQuery3 = "with <"+graphname+"> DELETE { ?o1 ?p2 ?o2 }  \r\n" + 
	              "WHERE { ?s ?p ?o. "
	              		+ " ?o ?p1 ?o1. "
	              		+ " ?o1 ?p2 ?o2. "
	              		+ "filter (?s = "+cweRes+")}";
	...
	public static boolean checkIsUptodate(String RMLTemp, String CWEXML, String CyberKnowledgeEp,String graphname) throws IOException {
		
	       
		Model CWETemp  = XMLParserJena.Parse(CWEXML, RMLTemp);
